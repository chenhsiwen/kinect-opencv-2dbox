...
class PolygonBlob extends Polygon2D {
  ...
  boolean  createBody() {
    ...
    if (getNumPoints() > 0) {
      ...
      for (int i=0; i<getNumPoints()/4; i++) {//change the factor to handle the collapse problem 
        Vec2D v = vertices.get(i*4);
        verts[i] = box2d.coordPixelsToWorld(int(v.x), int(v.y));
      }
      // create a chain from the array of vertices
      ChainShape chain = new ChainShape();
      if( verts.length > 0){  // how we 
